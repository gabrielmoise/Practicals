...
class UndoableEditor extends Editor with UndoHistory {
    ...
    override def transposeCommand: Boolean = {
        ...
        lastChange = new Change {
            override def undo(): Unit = ed.transpose(p)
            override def redo(): Unit = ed.transpose(p)
        }
        true
    }
    ...
    override def cutCommand: Boolean = {
        ...
        if (!super.cutCommand) return false

        val txt: Text.Immutable = ed.keepText

        lastChange = new Change {
            override def undo(): Unit = ed.insert(lstart, txt)
            override def redo(): Unit = ed.deleteRange(lstart, txt.length)
        }
        true
    }

    /** Command: Paste command */
    override def pasteCommand: Unit = {
        val txt: Text.Immutable = ed.keepText
        val pos: Int = ed.point
        super.pasteCommand
        lastChange = new Change {
            override def undo(): Unit = ed.deleteRange(pos, txt.length)
            override def redo(): Unit = ed.insert(pos, txt)
        }
    }
